...
public static void main(String[] args) {
        ...
        // 包下面的类
        Set<Class<?>> clazzs = getClasses("com.shu.yao");
        if (clazzs == null) {
            return;
        }
        ...
        for (Class<?> clazz : clazzs) {
        	 ...
        	 Method[] methodsw = clazz.getDeclaredMethods();
        	 Annotation[] annotations1;
        	 
        	 for(Method method:methodsw){
        		 annotations1 = method.getDeclaredAnnotations();
        		 for(Annotation ann:annotations1){
        			 System.out.println("方法名："+method.getName()+" 其上面的注解为："+ann.annotationType().getSimpleName());
        		 }
            ...
            for (Method method : methods) {
            	if(method.isAnnotationPresent(YaoRequestMapping.class)){
            		System.out.println("找到注解了---");
            	}
